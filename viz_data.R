...
sites_metadata |>
  ...
  geom_line() +
  labs(title = "Elevation of NJ River Sites", y = "Elevation (ft)", x = "Site Name")

plot_gage_height <- function(basin = stream_data_full$basin_name[2],
                             this_time = stream_data_full$datetime[1000]) {
  stream_data_full |>
    ...
    labs(
      title = paste(basin, "- Gage Height Change From Baseline"),
      subtitle = this_time,
...
plot_gage_height_over_time <- function(basin = stream_data_full$basin_name[2]) {
  stream_data_full |>
    ...
    geom_line() +
    labs(
      title = paste("Gage Height Over Time -", basin),
      x = "Datetime",
...
save_gage_height_plot <- function(basin = stream_data_full$basin_name[2],
                                  this_time = stream_data_full$datetime[100]) {
  ...
  time_diff <- sprintf("%04d", as.numeric(difftime(
    this_time, min(stream_data_full$datetime), units = "mins"
  )))
...
combine_gage_height_plots <- function(basin = BASIN) {
  ...
  files <- list.files(
    "img",
    pattern = paste0("gage_height_change_", basin, ".*.png"),
    full.names = TRUE
  )
...
get_inset_loc <- function(bbox,
  ...
  loc <- if (quadrant == "topright") {
    ...
    stop(
      "Invalid quadrant specified. Use 'topright', 'topleft', 'bottomright', or 'bottomleft'."
    )
  }
  return(loc)
}
...
custom_column <- function(this_time = times[1], inset = inset_loc) {
  ...
  size_precip_bar <- function(this_time = times[1],
                              rainfall = cum_rainfall,
                              inset = inset_loc) {
    # scale the cumulative precipitation to a range of 0 to 1
    inset_range <- inset_loc[4] - inset_loc[2] - .01
    scaled <- pull(filter(rainfall, datetime == this_time), precip_cum) /
      max(rainfall$precip_cum) * inset_range
    return(scaled)
  }
  ...
  list(
    ...
    annotate(
      ...
      label = paste0("Rain At EWR\n", pull(
        filter(cum_rainfall, datetime == this_time), precip_cum
      ), " in."),
      fontface = "bold",
      hjust = 0.5,
      vjust = 1,
...
map_gage_height <- function(this_time = stream_data_full$datetime[1]) {
  gg <- ggmap(basemap) +
    ...
    # add a title
    labs(
      title = "Northern NJ Flooding, July 14-15, 2025",
